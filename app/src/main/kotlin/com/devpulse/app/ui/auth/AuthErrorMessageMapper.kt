...
internal object AuthErrorMessageMapper {
    fun map(
        ...
        val detail =
            when (error.kind) {
                ...
                ApiErrorKind.NotFound -> "Сервис авторизации временно недоступен. Повторите попытку позже."
    ...
    private fun defaultBadRequestDetail(action: AuthAction): String {
        return when (action) {
            ...
            AuthAction.Register -> "Проверьте данные регистрации или попробуйте войти в существующий аккаунт."
        }
    }
